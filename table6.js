...
var svg3 = d3.select("#table6").append("svg")
.attr("width", width + margin.left + margin.right)
.attr("height", height + margin.top + margin.bottom)
.append("g")
.attr("transform", "translate(" + margin.left + "," + margin.top + ")");

//read the data for table 6
d3.csv("data_table6.csv", function(error, data) {
if (error) throw error;


var epiNames = d3.keys(data[0]).filter(function(key) { 		     return key !== "Type"; });
...
.append("text")
  ...
  .text("Annual Value ($) With and Without Cleanup");

var type = svg3.selectAll(".type")
  .data(data)
.enter().append("g")
  .attr("class", "type")
...
.on("mouseover", function(d, i) {
        ...
        div.html((d.Type)+"<br> "+(d.CurrentContry*100)+"%");
      })
